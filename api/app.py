...
def get_usuarios():
    conn = get_db_connection()
    usuarios = conn.execute('SELECT id, nome, email, cpf, telefone, cargo, status FROM usuarios ORDER BY nome').fetchall()
    conn.close()
...
def get_usuario(id):
    conn = get_db_connection()
    usuario = conn.execute('SELECT id, nome, email, cpf, telefone, cargo, status FROM usuarios WHERE id = ?', (id,)).fetchone()
    conn.close()
    if usuario is None:
...
def update_usuario(id):
    ...
    conn.execute('''
        ...
        WHERE id = ?
    ''', (data['nome'], data['email'], data.get('telefone'), data.get('status', 'ativo'), id))
    conn.commit()
    conn.close()
...
def get_usuarios_inativos():
    conn = get_db_connection()
    usuarios = conn.execute('SELECT id, nome, email, cpf, telefone, cargo, status FROM usuarios WHERE status = "inativo" ORDER BY nome').fetchall()
    conn.close()
    return jsonify([dict(usuario) for usuario in usuarios])
